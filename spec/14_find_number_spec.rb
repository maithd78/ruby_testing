...
describe FindNumber do
  ...
  describe '#make_guess' do
    ...
    context 'when min is 5 and max is 9' do
      subject(:game_guess_seven) { described_class.new(5, 9, number_guessing) }
      it 'returns 7' do
        guess = game_guess_seven.make_guess
        expect(guess).to eql(7)
      end
    end

    context 'when min is 8 and max is 9' do
      subject(:game_guess_eight) { described_class.new(8, 9, number_guessing) }
      it 'returns 8' do
        guess = game_guess_eight.make_guess
        expect(guess).to eql(8)
      end
    end
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      it 'is game over' do
        expect(game_over_true).to be_game_over
      end
    end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      it 'updates min to the same value as max' do
        ...
        expect(minimum).to eql(maximum)
      end

      it 'does not update max' do
        maximum = edge_guess_game.max

        expect(maximum).to eql(8)
      end
    end
  end
end

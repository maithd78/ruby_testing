...
describe BinaryGame do
  ...
  describe '#verify_input' do
    subject(:confirm_input) { described_class.new(1, 10) }
    ...
    context 'when given a valid input as argument' do
      it 'returns valid input' do
        min = confirm_input.instance_variable_get(:@minimum)
        max = confirm_input.instance_variable_get(:@maximum)
        valid_input = 5
        verify_method = confirm_input.verify_input(min, max, valid_input)
        expect(verify_method).to eql(valid_input)
      end
    end
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        ...
        max = confirm_input.instance_variable_get(:@maximum)
        invalid_input = 99
        verify_method = confirm_input.verify_input(min, max, invalid_input)
        expect(verify_method).to be_nil
      end
    end
  end
  ...
  describe '#display_binary_search' do
    ...
    context 'when game_over? is false five times' do
      before do
        allow(search_display).to receive(:game_over?).and_return(false, false, false, false, true)
      end
      it 'calls display_turn_order five times' do
        expect(game_display).to receive(:display_turn_order).with(search_display).exactly(4).times
        game_display.display_binary_search(search_display)
      end
    end
  end

...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      subject(:full_drink) { described_class.new('coffee') }

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(full_drink.type).to eq 'coffee'
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(full_drink.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    context 'when drink has 16 ounces or more' do
      # Create an explicit subject, using 'described_class' and your choice of
      # beverage type.
      subject(:juice){ described_class.new('juice')}

      # remove the 'x' before running this test
      it 'is full' do
        expect(juice).to be_full
      end
    end
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:latte) { described_class.new('latte',8) }

      # remove the 'x' before running this test
      it 'is not full' do
        expect(latte).not_to be_full
      end
    end
  end
end
